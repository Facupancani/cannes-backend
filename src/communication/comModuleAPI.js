...
router.post('/simular', (req, res) => {
  try {
    ...
    if (!req.body || !Array.isArray(req.body.habitaciones)) {
      console.log("El cuerpo de la solicitud no es válido o 'habitaciones' no es un array.");
    ...
    if (!Array.isArray(nuevasHabitaciones)) {
      ...
      return res.status(400).json({ error: 'Debes enviar un array de booleanos.' });
    }
    ...
    nuevasHabitaciones.forEach((estadoNuevo, i) => {
      if (estadoActual[i] !== estadoNuevo) {
        ...
        console.log(`Habitación ${room_number}: tarjeta ${estadoNuevo ? 'insertada' : 'retirada'}`);
      }
    });
